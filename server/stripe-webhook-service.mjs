 ...
 * @returns {Promise<boolean>}
 */
const markTransferBookingPaid = async (supabase, bookingId, session, paymentIntent, paymentKind) => {
 ...
 */
export const getPaidCheckoutSessionTarget = (event) => {
  ...
  return { session, metaPortal, metaTransfer, cref, paymentIntent, paymentKind }
}

/**
 * Stripe webhook: mark portal invoice or transfer paid once Stripe confirms the Checkout Session is paid.
...
export const handleStripeWebhook = async (rawBody, signatureHeader, env = process.env) => {
  ...
  const paidTarget = getPaidCheckoutSessionTarget(event)
  if (paidTarget) {
    ...
    if (paidTarget.metaPortal) {
      await markPortalInvoicePaid(supabase, paidTarget.metaPortal, paidTarget.paymentIntent, paidAt)
    } else if (paidTarget.metaTransfer) {
      await markTransferBookingPaid(
        supabase,
        paidTarget.metaTransfer,
        paidTarget.session,
        paidTarget.paymentIntent,
        paidTarget.paymentKind
      )
    } else if (paidTarget.cref) {
      const invOk = await markPortalInvoicePaid(supabase, paidTarget.cref, paidTarget.paymentIntent, paidAt)
      if (!invOk) {
        await markTransferBookingPaid(
          supabase,
          paidTarget.cref,
