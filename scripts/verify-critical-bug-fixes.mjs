...
import { selectPortalTripWorkspaceTarget } from '../server/portal-trip-workspace-save-service.mjs'
...
const verifyTripWorkspaceTargeting = () => {
  const rows = [
    { id: 'latest-real', config: { formKey: 'trip_service_cta', enquiryReferenceId: 'GSI-LATEST' } },
    { id: 'pending-draft', config: { formKey: 'trip_service_cta', enquiryReferenceId: '' } },
    { id: 'target', config: { formKey: 'trip_service_cta', enquiryReferenceId: 'GSI-TARGET' } }
  ]
  ...
  assert.equal(selectPortalTripWorkspaceTarget(rows, 'GSI-PENDING')?.id, 'pending-draft')
}

const verifyInvoiceSyncDoesNotMutateUnrelatedBooking = async () => {
  const tables = {
    portal_invoices: [
      {
        id: 'invoice-new-ref',
    ...
    transfer_bookings: [
      {
        ...
        created_at: '2026-06-23T09:59:00.000Z'
      }
    ],
...
verifyTripWorkspaceTargeting()
await verifyInvoiceSyncDoesNotMutateUnrelatedBooking()

console.log('verify-critical-bug-fixes: strict trip reference targeting and invoice sync passed')
