...
import { footerGroups, footerSocialLinks, heroBackgroundImage } from '../data/site-content'
...
function PackageAdminPage() {
  ...
  return (
    <div className="overflow-x-hidden bg-offwhite">
      ...
      <footer ref={footerRef} className="relative overflow-hidden bg-forest-950 px-6 py-8 text-white">
        <AmbientGolfBall className="right-[3%] top-2 opacity-75 xl:right-[6%]" size="sm" tone="footer" />
        <div className="mx-auto flex max-w-7xl flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-md">
            <Logo tone="hero" />
            <p className="mt-4 text-sm leading-relaxed text-white/60">
              Golf Sol Ireland creates premium Costa del Sol golf packages for Irish groups who want better planning, smarter pricing, and a smoother trip from first enquiry to final round.
            </p>
            <div className="mt-6">
                ...
                <ul className="mt-4 space-y-2 text-sm text-white/65">
                  {group.links.map((link) => (
                    <li key={link} className="flex items-center gap-2">
                      <ChevronRight className="h-4 w-4 text-gold-400" aria-hidden="true" />
                      <span>{link}</span>
                    </li>
                  ))}
                </ul>
        ...
        <div className="mx-auto mt-8 flex max-w-7xl flex-col gap-3 border-t border-white/10 pt-5 text-xs text-white/42 md:flex-row md:items-center md:justify-between">
          ...
          <p>Package planning and pricing for Irish golf groups travelling to the Costa del Sol.</p>
        </div>
      </footer>
    </div>
  )
