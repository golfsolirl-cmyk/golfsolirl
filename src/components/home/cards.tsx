...
export function TrustStat({ value, label }: StatItem) {
  return (
    <div className="min-w-[88px]">
      <p className="font-display text-[1.9rem] font-bold text-gold-400 md:text-[2.15rem]">{value}</p>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex h-full flex-col">
        ...
        <div className="mb-6">
          <h3 className={cx('text-[1.8rem] font-semibold tracking-tight md:text-[1.95rem]', isFeaturedPackage ? 'text-white' : 'text-forest-900')}>
            {name}
          </h3>
          <p className={cx('mt-3 text-base leading-8', isFeaturedPackage ? 'text-white/76' : 'text-forest-900/70')}>{description}</p>
        </div>
        ...
        <div className="mt-auto flex flex-col gap-4 border-t border-current/10 pt-5 sm:flex-row sm:items-center sm:justify-between">
          <p className={cx('text-sm font-semibold uppercase tracking-[0.12em]', isFeaturedPackage ? 'text-white/56' : 'text-forest-900/50')}>
    ...
    >
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <p className="font-display text-[1.9rem] font-bold text-white">{name}</p>
        </div>
      </div>

      <div className="p-6">
        <div className="mb-3 flex flex-wrap items-center gap-2 text-base text-forest-900/56">
          <span>{location}</span>
          <span className="h-1 w-1 rounded-full bg-forest-900/20" />
          <span>{distance}</span>
        </div>
        <p className="mb-4 text-base leading-8 text-forest-900/70">{description}</p>
        <div className="mb-5 flex flex-wrap gap-2">
          {tags.map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-fairway-100 bg-fairway-50 px-3 py-1.5 text-sm font-medium text-fairway-700"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="flex items-center justify-between border-t border-forest-100 pt-4">
          ...
          <a className="text-base font-semibold text-fairway-700 transition-colors hover:text-fairway-900" href="/packages">
            Request details
          </a>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <p className="font-display text-[1.9rem] font-bold text-white">{name}</p>
          <p className="text-base text-white/80">{area}</p>
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="mb-5 flex flex-wrap gap-2">
          {perks.map((perk) => (
            <span
              key={perk}
              className="rounded-full border border-forest-100 bg-forest-50 px-3 py-1.5 text-sm font-medium text-forest-900/72"
            >
              {perk}
            </span>
          ))}
        ...
        <div className="flex items-center justify-between border-t border-forest-100 pt-4">
          ...
          <a className="text-base font-semibold text-fairway-700 transition-colors hover:text-fairway-900" href={`/packages?stay=${tier}`}>
            Match with package
          </a>
        </div>
      </div>
...
export function FeatureTile({ title, description, icon: Icon }: TransferFeature) {
  return (
    ...
    >
      ...
      <h3 className="mb-2 text-[1.05rem] font-semibold text-white">{title}</h3>
      <p className="text-[1.02rem] leading-8 text-white/72">{description}</p>
    </motion.article>
  )
}
...
export function TestimonialCard({ quote, name, meta }: TestimonialItem) {
  return (
    ...
    >
      ...
      <p className="mb-5 text-base leading-8 text-forest-900/78">"{quote}"</p>
      <div>
        <p className="text-base font-semibold text-forest-900">{name}</p>
        <p className="text-base text-forest-900/56">{meta}</p>
      </div>
    </motion.article>
  )
}
