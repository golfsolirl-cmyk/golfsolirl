...
export function TrustStat({ value, label }: StatItem) {
  return (
    <div className="min-w-[88px]">
      <p className="font-display text-[2rem] font-bold text-gold-300 md:text-[2.35rem]">{value}</p>
      <p className="mt-1 text-base leading-7 text-white/76">{label}</p>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex h-full flex-col">
        ...
        <div className="mb-6">
          <h3 className={cx('text-[1.9rem] font-semibold tracking-[-0.02em] md:text-[2.05rem]', isFeaturedPackage ? 'text-white' : 'text-forest-900')}>
            {name}
          </h3>
          <p className={cx('mt-3 text-[1.03rem] leading-8', isFeaturedPackage ? 'text-white/78' : 'text-forest-900/72')}>{description}</p>
        </div>
    ...
    >
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <p className="font-display text-[1.95rem] font-bold tracking-[-0.02em] text-white">{name}</p>
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        <p className="mb-4 text-[1.03rem] leading-8 text-forest-900/72">{description}</p>
        <div className="mb-5 flex flex-wrap gap-2">
          {tags.map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-fairway-100 bg-fairway-50 px-3 py-1.5 text-sm font-medium text-forest-900"
            >
              {tag}
            </span>
          ))}
  ...
  return (
    ...
    >
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <p className="font-display text-[1.95rem] font-bold tracking-[-0.02em] text-white">{name}</p>
          <p className="text-[1.03rem] text-white/82">{area}</p>
        </div>
      </div>
...
export function TestimonialCard({ quote, name, meta }: TestimonialItem) {
  return (
    ...
    >
      ...
      <p className="mb-5 text-[1.03rem] leading-8 text-forest-900/80">"{quote}"</p>
      <div>
        <p className="text-[1.03rem] font-semibold text-forest-900">{name}</p>
