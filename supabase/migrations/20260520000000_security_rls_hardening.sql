...
-- Run via `supabase db push` or paste supabase/run-in-sql-editor-security-rls-hardening.sql in SQL Editor.

-- ---------------------------------------------------------------------------
-- 1) Enable (and force) RLS on all public heap tables missing it
-- ---------------------------------------------------------------------------
do $$
declare
  r record;
...
-- Force RLS on tables that store contact / account identifiers (even for table owners).
...
-- Idempotent: known app tables (covers tables that already had RLS but not FORCE).
...
-- ---------------------------------------------------------------------------
-- 2) profiles — email / phone; authenticated users only (no anon reads)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) enquiries — PII; admin + matching signed-in client only
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4) email anchors + auth blocks — admin only, authenticated role
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5) driver_calendar_bookings — customer PII; admin only
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 6) transport_form_public_flags — only non-PII flag row is anon-readable
-- ---------------------------------------------------------------------------
